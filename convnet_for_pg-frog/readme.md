...
If you want to prepare your own training images, you should start with  [FROG.ipynb](https://github.com/ksonod/scientific_works/blob/master/convnet_for_pg-frog/FROG.ipynb). If you want to train a convolutional neural network using training images generated by FROG.ipynb or stored in frog_train.zip, please check [CNN.ipynb](https://github.com/ksonod/scientific_works/blob/master/convnet_for_pg-frog/CNN.ipynb). 

# Background and importance 
Real-time observation of ultrafast chemical reactions is regarded as one of the ultimate goals of scientists because such observation can possibly give us deep insights about reaction dynamics, which are needed for controlling chemical reactions to get desired products efficiently. Because of the recent technical advances in laser development, ultrashort laser pulses whose temporal width is short enough to follow such ultrafast reaction dynamics in real time are now available. In order to know the temporal resolution that can be achieved by the ultrashort laser pulses, it is important to characterize them. Since these laser pulses are extremely short, it is impossible to measure the temporal width with electronic devices such as photodiodes. One of the most popular method for the characterization of the ultrashort laser pulses is frequency-resolved optical gating ([FROG](https://en.wikipedia.org/wiki/Frequency-resolved_optical_gating)). This method, however, usually requires complex retrieval algorithms. Here, a deep neural network (DNN) model is created to explore the possibility of fast and easy characterization of ultrashort laser pulses. The great advantage of the DNN model is that you can simply and quickly use the model for extracting parameters of the laser pulses without doing something complicated once training of the model is finished.
...
- In order to generate the PG-FROG traces (training images) by the numerical simulation, you should specify the temporal width of the laser pulse and chirp parameter. The temporal width of the laser pulse is determined by the duration when the laser pulse has high intensity. Precisely speaking, the pulse width is the full-width at half maximum (FWHM) of the intensity. The chirp parameter determines the temporal dependence of the frequency of the light.For example, the oscillating field of light in the graphical summary shows slow oscillation at first and then oscillates faster. These two parameters are target values of the training dataset.  
...
- As an initialization procedure, he_uniform is slightly better than other methods. 
